        ...
        <div class="uk-width-small-1-1 uk-width-medium-3-4 uk-width-large-7-10">
            <div id="index" class="bs uk-text-break">
                ...
                <div id="list" class="category">
                    <p class="date"><strong>${blog.name}</strong>目前共有标签：&nbsp;<strong>${fn:length(blog.label)}</strong>  </p>
                    <c:forEach items="${blog.label}" var="label">
                        <a id="lc" href="../q_action/select_label?id=${label.id}&username=${blog.username}"  data-uk-tooltip>${label.name}</a>
                    </c:forEach>
                </div>
            </div>
